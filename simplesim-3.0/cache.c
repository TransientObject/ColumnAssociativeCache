...
#define CACHE_TAG_PSEUDOASSOC(cp, addr) ((addr) >> ((cp)->tag_shift-1))  // we are not using this anywhere. instead whenever there's a column associative cache is defined, we are manipulating the tag shift value to get the desirable tag.
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  cp->writebacks = 0;
  cp->invalidations = 0;
  cp->pseudoassociative = 0; // initializing the flag to zero
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  blk->tag = 0;
	  blk->ready = 0;
      blk->rehash_bit |= 1;   // initializing the rehash bit to binary 1
...
cache_access(struct cache_t *cp,	/* cache to access */
	     enum mem_cmd cmd,		/* access type, Read or Write */ /* will be always read i guess */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  int swap = 0, loop = 0; // swap is to determine if there's a swap that is needed. loop is needed to determine if there's a loop going in rehash function. needed since i m not redefining the tag matching part
  md_addr_t temp_tag; // to swap the tags
  ...
  if (cp->hsize)
    {
      ...
      for (blk=cp->sets[set].way_head; blk; blk=blk->way_next)
	  {
        ...
        loop_start:
	      if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
	    	goto cache_hit;
        else if (!(blk->rehash_bit & 1) && cp->pseudoassociative && loop < 1)
          {
            //this is where we go to the rehashed location. Rbit = 1 is false
            set = set ^ (1 << (cp->tag_shift - cp->set_shift));
            blk = cp->sets[set].way_head;
            swap = 1; // if there's an initial miss and Rbit = 1 is false then swap has to be done compulsarily. Note that Rbit = 1 cannot be false the second time whatsoever.
            loop++;
            goto loop_start;
          }
        else if (cp->pseudoassociative)
          {
            if (swap == 0)
              blk->rehash_bit &= 0;
            else
  ...
  if (cp->hsize)
    ...
    if(swap == 1 && cp->pseudoassociative)
  {
    temp_tag = cp->sets[CACHE_SET(cp, addr)].way_head->tag;
    cp->sets[CACHE_SET(cp, addr)].way_head->tag = cp->sets[set].way_head->tag;
    cp->sets[set].way_head->tag = temp_tag;
    swap = 0;
  }
	    ...
	    tick_t now)			/* time of cache flush */
{
  ...
  for (i=0; i<cp->nsets; i++)
      ...
      for (blk=cp->sets[i].way_head; blk; blk=blk->way_next)
	{
      ...
      blk->rehash_bit |= 1; // setting rehash bit to 1 on flushing of cache. 
	}
    }
